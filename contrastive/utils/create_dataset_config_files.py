...
def replace_reference_yaml(crop_dir, side, ref, path):
...
def main(path, crop_path) -> None:
    ...
    for crop_dir in crop_dirs:
        for side in ['L', 'R']:
            filedata, result_file = replace_reference_yaml(crop_dir, side, ref, path)
            print(result_file)
            with open(result_file, 'w') as file:
                file.write(filedata)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process dataset configs.")
    parser.add_argument("--path", type=str, required=True, help="Path to the configs directory")
    parser.add_argument("--crop_path", type=str, required=True, help="Path to the crops directory")
    args = parser.parse_args()
    main(args.path, args.crop_path)
